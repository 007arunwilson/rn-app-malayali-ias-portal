...
const Categories = (props) => {
    ...
    const { categories, activeCategoryId } = useSelector((state) => state.exam.running);

    if (categories.length < 2) return null; // Categories will only display if there is multiple

    const onCategorySelect = (category) => dispatch(setActiveQuestionByIndex(category.questionsIndex[0]));
