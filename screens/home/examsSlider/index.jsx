...
const ExamsSlider = () => {
    ...
    React.useEffect(() => {
        getPackageExamsWithUserAttempt({ params: { page: 1, limit: 5 }, urlParams: { packageId: activePackageId } })
            .then(result => {
                setItems(result);
                setLoaded(true);
            });
    ...
    return loaded && items.length ? (
        <View style={styles.container}>
            ...
            <FlatList
                data={items}
                renderItem={({ item, index }) => <Item onExamSelect={onExamSelect} item={item} index={index} />}
